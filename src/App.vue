...
export default {
  ...
  data(){
    {
      return {
        menuList:[
          {id:0, itemlist: 'Clover',active: true},
  ...
  setTimeout(() => {
    alert(
        ...
        " необходимо убедиться, что уже выполнялалсь команда" +
        " psql-sand в терминале после перезагрузки компьютера" +
        " (psql-sand требует ввода пароля .rsa ключа при перезгрузки компьютера)"
    )
  },2000)
</script>
...
.btn:hover {
  background-image: linear-gradient(20deg, #0b7fec, #29a9f8);
}

.btn_disabled {
  background-image: linear-gradient(40deg, #757c83, #7892a1);
}
.input {
  height: 27px;
  width: 250px;
